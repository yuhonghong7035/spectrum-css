...
function CSSVariableEditor() {
  ...
  window.addEventListener('contextmenu', e => {
    if (this._inspectMode) {
      if (e.target === this._inspectangle) {
...
<div class="spectrum-Site-panel is-open CSSVariableEditor-container spectrum--dark spectrum" id="editorContainer">
  ...
  if (activeEl) {
    ...
    if (activeEl && !CSSVariableEditor.isBanned(activeEl.className)) {
      if (activeEl === this._inspectedElement) {
        ...
        this._inspectangle.style.width = rect.width + 'px';
        this._inspectangle.style.height = rect.height + 'px';
        this._inspectangle.hidden = false;
        this._inspectangleLabel.innerText = componentName;

        if (rect.top - 80 < 0 && rect.bottom < window.innerHeight) {
...
CSSVariableEditor.prototype.inspect = function(element) {
  ...
  this._componentCrumbLabel.innerText = CSSVariableEditor.getComponentNameFromElement(element).replace(/^spectrum-/, '');
